...
// encoder returns a statement which accesses a data field, i.e. msgData.Int32(17).
func (w *writer) encoder(typ *compiler.Type, data string, i string) string {
	kind := typ.Kind
	...
	case compiler.KindByte:
		return fmt.Sprintf(`%v.Byte(%v)`, data, i)
	...
	case compiler.KindMessage:
		getFunc := messageGetFunc(typ)
		w.linef(`v, _ := %v(%v)`, getFunc, data)
		return "v"
